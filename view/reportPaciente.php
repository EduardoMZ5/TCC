<?php
  ...
  <body>
    <h1 style="text-align: center; font-size: 30px;">Pacientes Cadastrados</h1>
  ...
  if (sqlsrv_num_rows($qyRelatorio) != 0) {
    ...
    $html .= '
     <table cellspacing="0.2" style="width: 100%; table-layout: fixed;">
      <tr>
        <td style="width: 32%;"><b>Nome</b></td>
        <td style="width: 35%;"><b>Email</b></td>
        <td style="width: 15%;"><b>Nascimento</b></td>
        <td style="width: 18%;"><b>Celular</b></td>        
      </tr>
      <tr>
    ...
    while ($rows = sqlsrv_fetch_array($qyRelatorio)) {
      ...
      $html.= '<tr>';	 
	    ...
	    $html.= ' <td style=" overflow: hidden;">'.$rows['DataNascimento']->format('d/m/Y').'</td>'; 	 
	    $html.= ' <td style=" overflow: hidden;">'.$rows['TelefoneCelular'].'</td>'; 
	    $html.= '</tr>
               <tr>
                 <td colspan="'.$numColumn.'">
